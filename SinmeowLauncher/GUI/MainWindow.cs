...
namespace AmarothLauncher
{
    public partial class MainWindow : Form
    {
        ...
        {
            try { Icon = new Icon("SinmeowgraLauncher.ico"); } catch { }
        ...
        private void UpdateSelf()
        {
            ...
            if (File.Exists(cwd + "\\OldSinmeowgraLauncher.exe"))
                File.Delete(cwd + "\\OldSinmeowgraLauncher.exe");
            if (File.Exists(cwd + "\\OldLauncherConfig.xml"))
                File.Delete(cwd + "\\OldLauncherConfig.xml");
            if (File.Exists(cwd + "\\OldSinmeowgraLauncher.ico"))
                File.Delete(cwd + "\\OldSinmeowgraLauncher.ico");

            // Backup current files.
            if (File.Exists(exeName))
                File.Move(exeName, cwd + "\\OldSinmeowgraLauncher.exe");
            if (File.Exists(cwd + "\\LauncherConfig.xml"))
                File.Move(cwd + "\\LauncherConfig.xml", cwd + "\\OldLauncherConfig.xml");
            if (File.Exists(cwd + "\\SinmeowgraLauncher.ico"))
                File.Move(cwd + "\\SinmeowgraLauncher.ico", cwd + "\\OldSinmeowgraLauncher.ico");
            try
            {
                if (File.Exists(cwd + "\\SinmeowgraLauncher.zip"))
                    File.Delete(cwd + "\\SinmeowgraLauncher.zip");
                // Download new Launcher.
                using (var client = new AmWebClient(3000))
                {
                    client.DownloadFile(c.SubElText("Paths", "LauncherPath"), cwd + "\\SinmeowgraLauncher.zip");
                }

                // Unzip new Launcher.
                Shell32.Shell objShell = new Shell32.Shell();
                Shell32.Folder destinationFolder = objShell.NameSpace(cwd);
                Shell32.Folder sourceFile = objShell.NameSpace(cwd + "\\SinmeowgraLauncher.zip");
            ...
            catch (Exception e)
            {
                ...
                if (File.Exists(cwd + "\\OldSinmeowgraLauncher.exe"))
                    File.Move(cwd + "\\OldSinmeowgraLauncher.exe", exeName);
                if (File.Exists(cwd + "\\OldLauncherConfig.xml"))
                    File.Move(cwd + "\\OldLauncherConfig.xml", cwd + "\\LauncherConfig.xml");
                if (File.Exists(cwd + "\\OldSinmeowgraLauncher.ico"))
                    File.Move(cwd + "\\OldSinmeowgraLauncher.ico", cwd + "\\SinmeowgraLauncher.ico");
