...
namespace AmarothLauncher.Core
{
    ...
    public class Config
    {
        ...
        private void MainSettingsDefault()
        {
            ...
            AddSubnodeDefault(node, "KeepBlizzlikeMPQs", "0", "1 or 0. If 1, blizzlike MPQs in Data are ignored by Launcher. Otherwise they are handled in a same manner as custom one. Recommended 1.");
        ...
        private void PathConfigDefault()
        {
            ...
            AddSubnodeDefault(node, "FilelistPath", "https://sinmeowgra.com/launcherination/filelist.conf", "Path to text filelist.");
            AddSubnodeDefault(node, "VersionPath", "https://sinmeowgra.com/launcherination/launcherversion.conf", "Path to text file which contains your Lancher's current version (version is a double value with . as separator!");
            AddSubnodeDefault(node, "LauncherPath", "https://sinmeowgra.com/launcherination/SinmeowgraLauncher.zip", "Path to a zip file with Launcher files - used if Launcher finds itself outdated.");
            AddSubnodeDefault(node, "FilesRootPath", "https://sinmeowgra.com/launcherination/", "Path to folder with files. Paths in filelist are relative to this path.");
            AddSubnodeDefault(node, "ChangelogPath", "https://sinmeowgra.com/launcherination/changelog.xml", "!HTTP! path to changelog XML file.");
            AddSubnodeDefault(node, "ChangelogFTPPath", "ftp://ftp.sinmeowgra.com//home/sinmeowg/sinmeowgra.com/launcherination", "!Full! !FTP! path to folder in which changelog is. Notice that //www/ part. You may want to use an IP instead of a domain name.");
            AddSubnodeDefault(node, "Webpage", "https://sinmeowgra.com/", "URL which is to be opened when user clicks on Project webpage button.");
            AddSubnodeDefault(node, "Registration", "https://sinmeowgra.com/account/register/", "URL which is to be opened when user clicks on Registration button.");
            AddSubnodeDefault(node, "Instructions", "https://sinmeowgra.com/launcherination/manual.html", "URL which is to be opened when user clicks on Launcher manual button.");
            AddSubnodeDefault(node, "HelloImage", "https://sinmeowgra.com/api/keep-update-img.php", "URL to image which is to be displayed in Main window (latest news image). Clicking on it opens a changelog browser.");
        ...
        private void MainWindowDefault()
        {
            ...
            AddSubnodeDefault(node, "WindowName", "Sinmeowgra's Launcher " + version.ToString("F", CultureInfo.InvariantCulture), "Name of main window. Change this to match your project's name.");
        ...
        private void MessagesDefault()
        {
            ...
            AddSubnodeDefault(node, "NoNeedUpdate", "[Patcher MPQs Updater] Update is not requested, this feature was enabled to prevent HD Client Patch removed.");
